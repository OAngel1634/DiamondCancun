    ...
    try {
        if ($id > 0) {
            ...
            $stmt->execute([
                ...
                $correo, $telefono, $rfc, $id
            ]);
            $mensaje = "Cliente actualizado correctamente";
            $mensaje_tipo = 'success';
        } else {
            // SOLUCIÓN: Generar nuevo ID para cliente
            $stmtMax = $conn->query("SELECT MAX(Id_Cliente) AS max_id FROM Cliente");
            $maxId = $stmtMax->fetch(PDO::FETCH_ASSOC)['max_id'];
            $newId = $maxId ? $maxId + 1 : 1;

            // Crear nuevo cliente con ID generado
            $stmt = $conn->prepare("INSERT INTO Cliente (
                Id_Cliente, Nombre, Apellido_P, Apellido_M, 
                Correo, Telefono, RFC
            ) VALUES (?, ?, ?, ?, ?, ?, ?)");
            
            $stmt->execute([
                $newId, $nombre, $apellido_p, $apellido_m, 
                $correo, $telefono, $rfc
            ]);
            $mensaje = "Cliente creado correctamente (ID: $newId)";
            $mensaje_tipo = 'success';
        }
...
<head>
    ...
    <title>CRUD Clientes - Diamond Bright</title>
...
<body>
    ...
    <div class="sidebar">
        ...
        <ul class="nav-links">
            ...
            <li><a href="clientes.php" class="active"><i class="bi bi-person-lines-fill"></i> <span>Clientes</span></a></li>
    ...
    <div class="main-content">
        <div class="container-fluid">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h2 class="mb-0"><i class="bi bi-person-lines-fill me-2"></i>Administración de Clientes</h2>
                <?php if ($cliente_editar): ?>
                    <span class="badge bg-warning text-dark status-badge">
                        <i class="bi bi-pencil"></i> Editando Cliente #<?= $cliente_editar['Id_Cliente'] ?>
                    </span>
                <?php else: ?>
            ...
            <!-- Formulario CRUD -->
            <div class="card mb-4 <?= $cliente_editar ? 'edit-mode' : '' ?>">
                <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">
                        <i class="bi bi-person-plus me-2"></i>
                        <?= $cliente_editar ? 'Editar Cliente' : 'Agregar Nuevo Cliente' ?>
                    </h5>
                    <?php if ($cliente_editar): ?>
                        <span class="badge bg-white text-dark">ID: <?= $cliente_editar['Id_Cliente'] ?></span>
                    <?php endif; ?>
                </div>
                <div class="card-body">
                    <form method="POST">
                        <input type="hidden" name="id" value="<?= $cliente_editar['Id_Cliente'] ?? '' ?>">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="text" class="form-control" name="apellido_p" 
                                    value="<?= htmlspecialchars($cliente_editar['Apellido_P'] ?? '') ?>">
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">Apellido Materno</label>
                                <input type="text" class="form-control" name="apellido_m" 
                                    value="<?= htmlspecialchars($cliente_editar['Apellido_M'] ?? '') ?>">
                            </div>
                            
                            <div class="col-md-6">
                                <label class="form-label">Correo</label>
                                <input type="email" class="form-control" name="correo" 
                                    value="<?= htmlspecialchars($cliente_editar['Correo'] ?? '') ?>">
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" name="rfc" 
                                    value="<?= htmlspecialchars($cliente_editar['RFC'] ?? '') ?>" maxlength="13">
                            </div>
                        </div>
                        ...
                        <div class="mt-4 d-flex justify-content-between">
                            <div>
                                ...
                                <?php if ($cliente_editar): ?>
                                    ...
                                    ?>
                                    <a href="clientes.php?<?= $queryParams ?>" class="btn btn-outline-secondary">
            ...
            <div class="card">
                <div class="card-header bg-secondary text-white d-flex justify-content-between align-items-center">
                    <h5 class="mb-0"><i class="bi bi-list-task me-2"></i>Lista de Clientes</h5>
                    <span class="badge bg-light text-dark">
                        <?= count($clientes) ?> <?= count($clientes) === 1 ? 'Registro' : 'Registros' ?>
                    </span>
                </div>
                <div class="card-body">
                                    ...
                                    <?php foreach ($clientes as $cliente): ?>
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <?php if ($cliente_editar && $cliente_editar['Id_Cliente'] == $cliente['Id_Cliente']): ?>
                                                    <span class="badge bg-warning text-dark ms-2">Editando</span>
                                                <?php endif; ?>
                                            </td>
                                            <td>
                                                <?= htmlspecialchars($cliente['Apellido_P']) ?> <?= htmlspecialchars($cliente['Apellido_M']) ?>
                                            </td>
                                            ...
                                            <td>
                                                ...
                                                <div class="action-buttons">
                                                    <a href="clientes.php?editar=<?= $cliente['Id_Cliente'] ?>&<?= $queryParams ?>" 
                    ...
                    <?php else: ?>
                        <div class="empty-state">
                            ...
                            <p class="text-muted">Intenta cambiar tus filtros o agregar un nuevo cliente</p>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            deleteModal.addEventListener('show.bs.modal', function(event) {
                ...
                const deleteUrl = `clientes.php?eliminar=${id}&filtro=${filtro}&campo=${campo}`;
